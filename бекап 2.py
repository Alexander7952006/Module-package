    ...
    for key, value in dct.items():
        ...
        for el in columns[iteration]:
            if el == 'ИСТИНА':
                ...
                lst.append(False)
            elif (fnmatch(el, '*') and el.count('0') + el.count('1') + el.count('2') + el.count('3') + el.count('4') + el.count('5') + el.count('6') + 
...
def set_column_types(types_dict, by_number=True):
    ...
    for indx in range(len(res_columns[0])):
        ...
        for column in res_columns:
            if column[indx] == True and type(column[indx])== bool:
                row.append('ИСТИНА')
            elif column[indx] == False and type(column[indx])== bool:
                row.append('ЛОЖЬ')
            else:
